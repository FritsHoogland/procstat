...
pub async fn add_vmstat_to_history(
    ...
    // this is for values in vmstat that represent absolute current state
    // and NOT A COUNTER!
    macro_rules! generate_assignments_for_history_addition_last_value {
        ($($field_name:ident),*) => {
            $(
                let $field_name = statistics.get(&("vmstat".to_string(), "".to_string(), stringify!($field_name).to_string())).unwrap_or(&Statistic::default()).last_value;
            )*
        };
    }

    generate_assignments_for_history_addition_last_value!(
        nr_free_pages,
        nr_zone_inactive_anon,
        nr_zone_active_anon,
        nr_zone_inactive_file,
