...
pub async fn archiver() {
    ...
    let mut archive_time = Local::now().duration_trunc(Duration::minutes(10)).unwrap()+Duration::minutes(10);
    ...
    loop {
        ...
        if Local::now() > archive_time {
            ...
            archive_time += Duration::minutes(10);
            debug!("new archive_time: {:?}", archive_time);
        };
    }
}
pub fn archive(high_time: DateTime<Local>) {
    let mut transition = HistoricalDataTransit::default();
    let low_time = high_time-Duration::minutes(10);
    debug!("archive times: low: {:?}, high {:?}", low_time, high_time);
