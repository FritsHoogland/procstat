...
pub fn create_networkdevice_plot(buffer: &mut [u8], device_name: String) {
...
fn networkdevice_mbit_plot(
    ...
    let high_value_mbit = historical_data_read
        ...
        .filter(|networkdevices| networkdevices.device_name == device_name)
        .map(|networkdevices| {
            ((networkdevices.receive_bytes + networkdevices.transmit_bytes) / (1024_f64 * 1024_f64))
                * 8_f64
                * 1.1_f64
        })
    ...
    let min_total_mbit = historical_data_read
        .iter()
        .filter(|networkdevice| {
            networkdevice.device_name == device_name
                && (networkdevice.transmit_bytes + networkdevice.receive_bytes) > 0_f64
        })
        .map(|networkdevice| {
            ((networkdevice.transmit_bytes + networkdevice.receive_bytes) / (1024_f64 * 1024_f64))
                * 8_f64
        })
    ...
    contextarea
        .draw_series(LineSeries::new(
            historical_data_read
                ...
                .map(|networkdevice| {
                    (
                        networkdevice.timestamp,
                        ((networkdevice.transmit_bytes + networkdevice.receive_bytes)
                            / (1024_f64 * 1024_f64))
                            * 8_f64,
                    )
                }),
    ...
    let min_transmit_mbit = historical_data_read
        .iter()
        .filter(|networkdevice| {
            networkdevice.device_name == device_name && networkdevice.transmit_bytes > 0_f64
        })
            ...
            historical_data_read
                ...
                .map(|networkdevice| {
                    Circle::new(
                        (
                            networkdevice.timestamp,
                            networkdevice.transmit_bytes / (1024_f64 * 1024_f64) * 8_f64,
                        ),
                        4,
                        RED.filled(),
                    )
    ...
    let min_receive_mbit = historical_data_read
        .iter()
        .filter(|networkdevice| {
            networkdevice.device_name == device_name && networkdevice.receive_bytes > 0_f64
        })
            ...
            historical_data_read
                ...
                .map(|networkdevice| {
                    Circle::new(
                        (
                            networkdevice.timestamp,
                            networkdevice.receive_bytes / (1024_f64 * 1024_f64) * 8_f64,
                        ),
                        3,
                        GREEN.filled(),
                    )
...
fn networkdevice_packet_plot(
    ...
    let high_value_packets = historical_data_read
        ...
        .filter(|networkdevices| networkdevices.device_name == device_name)
        .map(|networkdevices| {
            (networkdevices.receive_packets + networkdevices.transmit_packets) * 1.1_f64
        })
        .max_by(|a, b| a.partial_cmp(b).unwrap())
        .unwrap_or_default();
    let latest = historical_data_read
    ...
    let min_total_packets = historical_data_read
        ...
        .filter(|networkdevice| {
            networkdevice.device_name == device_name
                && (networkdevice.transmit_packets + networkdevice.receive_packets) > 0_f64
        })
        ...
        .draw_series(LineSeries::new(
            historical_data_read
                ...
                .map(|networkdevice| {
                    (
                        networkdevice.timestamp,
                        networkdevice.transmit_packets + networkdevice.receive_packets,
                    )
                }),
            BLACK,
        ))
    ...
    let min_transmit_packets = historical_data_read
        .iter()
        .filter(|networkdevice| {
            networkdevice.device_name == device_name && networkdevice.transmit_packets > 0_f64
        })
    ...
    contextarea
        .draw_series(
            historical_data_read
                ...
                .filter(|networkdevice| {
                    networkdevice.device_name == device_name
                        && networkdevice.transmit_packets > 0_f64
                })
                .map(|networkdevice| {
                    Circle::new(
                        (networkdevice.timestamp, networkdevice.transmit_packets),
                        4,
                        RED.filled(),
                    )
                }),
    ...
    let min_receive_packets = historical_data_read
        .iter()
        .filter(|networkdevice| {
            networkdevice.device_name == device_name && networkdevice.receive_packets > 0_f64
        })
    ...
    contextarea
        .draw_series(
            historical_data_read
                ...
                .filter(|networkdevice| {
                    networkdevice.device_name == device_name
                        && networkdevice.receive_packets > 0_f64
                })
                .map(|networkdevice| {
                    Circle::new(
                        (networkdevice.timestamp, networkdevice.receive_packets),
                        3,
                        GREEN.filled(),
                    )
                }),
...
fn networkdevice_error_plot(
    ...
    let mut contextarea = ChartBuilder::on(&multi_backend[backend_number])
        ...
        .set_label_area_size(LabelAreaPosition::Right, LABEL_AREA_SIZE_RIGHT)
        .caption(
            format!("Networkdevice: {} errors per second", device_name),
            (CAPTION_STYLE_FONT, CAPTION_STYLE_FONT_SIZE),
        )
