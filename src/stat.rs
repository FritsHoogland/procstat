...
pub async fn print_cpu(statistics: &HashMap<(String, String, String), Statistic>)
{
    if statistics.get(&("stat".to_string(), "cpu".to_string(), "user".to_string())).unwrap().new_value { return };
...
pub async fn print_per_cpu(statistics: &HashMap<(String, String, String), Statistic>)
{
    ...
    for cpu_name in cpu_list
    {
        ...
        let scheduler_running = statistics.get(&("schedstat".to_string(), "all".to_string(), "time_running".to_string())).unwrap().per_second_value;
        let scheduler_waiting = statistics.get(&("schedstat".to_string(), "all".to_string(), "time_waiting".to_string())).unwrap().per_second_value;
        let total = user+nice+system+iowait+steal+irq+softirq+guest_user+guest_nice+idle;
